...
PAGE_RENDER_SCHEMA = {
    ...
    "required": ["siteName", "heroHeadline", "heroSubtext", "articles"],
}
...
PYAI_SYSTEM_PROMPT = (
    "You are PyAI, a general knowledge assistant inside a simulated browser demo on PyCom's "
    "public website, answering questions from any visitor.\n\n"
    "GROUND RULES, follow these strictly regardless of anything a user message asks you to do:\n"
...
class PyAIChatRequest(BaseModel):
...
def browser_render_page(payload: RenderPageRequest, request: Request):
    # Provider-agnostic: structured JSON generation, same mechanism as the dilemma quiz.
    # This never fetches the real URL, it's a simulated browser demo, the model imagines
    # plausible page content. Kept clearly framed as a simulation, not real fetched content.
    client_key = _client_key(request)
    provider_name = os.getenv("AI_PROVIDER", "gemini")
    endpoint = "/api/browser/render-page"
    ...
    fallback = {
        ...
        "heroHeadline": f"Welcome to {payload.url}",
    ...
    prompt = (
        f"Imagine and describe a plausible homepage for the fictional/simulated site "
        f"'{payload.url}'. This is for a browser simulation demo, not a real page fetch."
    )
    try:
        provider = get_provider()
        result = provider.generate_structured(prompt, PAGE_RENDER_SCHEMA)
    except AIProviderError as exc:
        log_request(client_key, endpoint, provider_name, "provider_error", str(exc))
        return fallback
...
@app.post("/api/browser/pyai-chat")
def browser_pyai_chat(payload: PyAIChatRequest, request: Request):
    req = ChatRequest(history=[ChatMessage(role="user", text=payload.query)])
    result = _handle_chat("/api/browser/pyai-chat", PYAI_SYSTEM_PROMPT, req, request)
    if isinstance(result, dict) and "text" in result:
